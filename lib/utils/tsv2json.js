import { indexOf, tail } from 'lodash';
import { readFileSync } from 'fs';
import headers, { isHeaderValid } from './headers';
const response = {
  json: {},
  error: { message: '' },
};
...
const getItemFromRow = (fields, header, headerString) => {
  return fields[indexOf(header, headerString)] || '';
};
...
const getProductUrlFromLineItem = (fields, header) => {
  try {
    ...
    response.error.message = getItemFromRow(fields, header, 'Row ID')
      ? response.error.message +
        `\nError at Row Id ${getItemFromRow(fields, header, 'Row ID')}. ${
          e.message
        }`
      : response.error.message + incorrectHeaderText;
    return '';
  }
};
...
const getPriceFromSales = (fields, header) => {
  try {
    ...
    if (isNaN(price)) {
      response.error.message = getItemFromRow(fields, header, 'Row ID')
        ? response.error.message +
          `\nError at Row Id ${getItemFromRow(
            fields,
            header,
            'Row ID',
          )}. Please enter a valid number for sales price, entered price is : ${getItemFromRow(
            fields,
            header,
            'Sales',
          )}`
...
const getISOOrderDateIfAfter = (fields, header, dateString) => {
  try {
    ...
    if (orderDate && new Date(orderDate) > new Date(dateString)) {
      ...
      response.error.message = getItemFromRow(fields, header, 'Row ID')
        ? response.error.message +
          `\nError at Row ${getItemFromRow(
            fields,
            header,
            'Row ID',
          )}. Invalid date, Input date:${orderDate}. Hence not parsed`
        : response.error.message + incorrectHeaderText;
    }
  } catch (e) {
    response.error.message =
